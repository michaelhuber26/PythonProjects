...
def is_emergency(event):
    ...
    if event.code == 317 and direction == "down":  # emergency tap-down on the touchpad
        ...
        emergency_tap_time = int(
            round(time.time() * 1000)
        )  # set now as the most recent tap time
        if emergency_tap_time < (
            previous_tap + MAX_EMERGENCY_DELAY
        ):  # if the current tap-down comes less than x milliseconds after last tap
            return True
    return False
...
if __name__ == "__main__":
    ...
    for event in gamepad.read_loop():
        ...
        if (
            event.type == ecodes.EV_KEY and event.code in button_presses
        ):  # any button press other than leftpad
